...
class DirectDebitInstallment(db.Model):
    """Installments generated by a direct debit schedule (scheduled vs paid)."""
    __tablename__ = 'direct_debit_installments'

    id = db.Column(db.Integer, primary_key=True)
    # Link back to our schedule and booking for easy joins
    schedule_id = db.Column(db.String(100), db.ForeignKey('direct_debit_schedules.schedule_id'), nullable=False)
    booking_id = db.Column(db.Integer, db.ForeignKey('bookings.id'), nullable=False)

    # Pay Advantage identifiers
    external_payment_id = db.Column(db.String(150), unique=True)
    ...
    # Relationships
    schedule = db.relationship('DirectDebitSchedule', primaryjoin="DirectDebitInstallment.schedule_id==DirectDebitSchedule.schedule_id", backref='installments', lazy=True)
    booking = db.relationship('Booking', backref='direct_debit_installments', lazy=True)

    def __repr__(self):
        return f"<DirectDebitInstallment {self.schedule_id} {self.due_date} {self.status}>"
